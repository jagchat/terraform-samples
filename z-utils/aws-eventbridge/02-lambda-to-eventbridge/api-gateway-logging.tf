...
resource "aws_api_gateway_method_settings" "general_settings" {
  ...
  stage_name  = aws_api_gateway_deployment.apiLambdaDeployment.stage_name
  method_path = "*/*"
...
resource "aws_cloudwatch_log_group" "gateway-log-group" {
  ...
  name              = "API-Gateway-Execution-Logs_${aws_api_gateway_rest_api.apiLambda.id}/${aws_api_gateway_deployment.apiLambdaDeployment.stage_name}"
  retention_in_days = 1
}
